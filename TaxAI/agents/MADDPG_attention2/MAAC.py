...
class maddpg_agent:
    def __init__(self, envs, args):
        ...
        self.model_path, _, curr_run = make_logpath(algo="maddpga",n=self.args.n_households)
        ...
        if self.wandb:
            wandb.init(
                ...
                project="taxing",
                entity="ggccccai",
                name=self.model_path.parent.parent.name + "-"+ self.model_path.parent.name + "-"+ str(self.args.hidden_size) + "-"+ str(self.envs.gov_task) + "-"+ str(self.args.batch_size) + f" {curr_run}",
        ...
        # for epoch in range(1):
            ...
            for t in range(self.args.epoch_length):
                ...
                for i in range(self.args.agent_block_num-1):
                    if i == 0:
                        num = num_set[:1 if 0<0.1*self.envs.households.n_households<1 else int(0.25*self.envs.households.n_households)]
                    elif i == 1:
                        num = num_set[1 if 0<0.1*self.envs.households.n_households<1 else int(0.25*self.envs.households.n_households):int(0.5*self.envs.households.n_households)]
                    else:
            ...
            # for _ in range(self.args.update_cycles):
                # if t % 10 == 0:
                if t % self.args.update_cycles == 0:
            ...
            if epoch % self.args.display_interval == 0:
                ...
                print('[{}] Epoch: {} / {}, Frames: {}, gov_Rewards: {:.3f}, house_Rewards: {:.3f}, years:{:.3f}, actor_loss: {:.3f}, critic_loss: {:.3f}'.format(
            ...
            if epoch % self.args.save_interval == 0:
                os.makedirs(str(self.model_path) + f"/{epoch}", exist_ok=True)
                for agent_i in range(len(self.agents)):
                    torch.save(self.agents[agent_i].actor_network.state_dict(), str(self.model_path) + f"/{epoch}" + '/agent_'+str(agent_i)+'.pt')
