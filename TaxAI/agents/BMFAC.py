...
class BMFAC_agent:
    def __init__(self, envs, args):
        ...
        self.model_path, _, curr_run = make_logpath(algo="bmfac",n=self.args.n_households)
        ...
        if self.wandb:
            wandb.init(
                ...
                project="taxing",
                entity="ggccccai",
                name=self.model_path.parent.parent.name + "-"+ self.model_path.parent.name + "-"+ str(self.args.hidden_size) + "-"+ str(self.envs.gov_task) + f" {curr_run}",
    ...
    def multiple_households_mean_action(self, actions=None):
        ...
        if actions is None:
            ...
            top10_wealth_index = sorted_wealth_index[:1 if 0<0.1*self.args.n_households<1 else int(0.25*self.args.n_households)]
