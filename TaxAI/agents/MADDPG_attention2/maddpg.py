...
class MADDPG:
    def __init__(self, args, agent_id):  # 因为不同的agent的obs、act维度可能不一样，所以神经网络不同,需要agent_id来区分
        ...
        critic_act_size = self.args.gov_action_dim + self.args.house_action_dim * self.args.n_households
        critic_obs_size = self.args.gov_obs_dim + (self.args.house_obs_dim-self.args.gov_obs_dim) * self.args.n_households
        ...
        self.critic_network = Critic(critic_obs_size, critic_act_size, hidden_size=args.hidden_size)
        self.critic_target_network = Critic(critic_obs_size, critic_act_size, hidden_size=args.hidden_size)
    ...
    def train(self, transitions, other_agents):
        ...
        num = []
        num.append(num_set[:1 if 0<0.1*self.args.n_households<1 else int(0.25*self.args.n_households)])
        num.append(num_set[1 if 0<0.1*self.args.n_households<1 else int(0.25*self.args.n_households):int(0.5 * self.args.n_households)])
