...
class economic_society:
    ...
    def gov_reward(self):
        ...
        elif gov_goal == "gdp_gini_trans":
            # 使用softplus将奖励转为非负但保留单调性，间接来奖励长期运行
            return np.log(1 + np.exp((self.per_household_gdp - self.old_per_gdp) / self.old_per_gdp - self.gini_weight * (self.wealth_gini * self.income_gini)))
